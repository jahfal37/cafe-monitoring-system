...
async function loadCameras(cafeId) {
  ...
  const res = await fetch(
    `http://localhost:5000/api/devices?cafe_id=${cafeId}`,
  );
  ...
  cameras.forEach((cam, index) => {
    container.innerHTML += `
        <div class="bg-white rounded-cafe shadow-cafe-card p-4">
            ...
            <div class="relative w-full h-100 bg-black rounded-xl overflow-hidden">
  ...
  fetch(`http://127.0.0.1:5000/api/services?tanggal=${tanggal}`, {
    ...
    .then((data) => {
      ...
      data.data.forEach((item) => {
        ...
        const statusText = item.status === "long" ? "Long wait" : "Normal";
