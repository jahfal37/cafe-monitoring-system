...
HEF_PATH = config.get("hef_path", "model/best.hef")
LABELS_PATH = config.get("labels_path", "model/labels.json")

print("[HEF PATH]:", HEF_PATH)
print("[LABELS PATH]:", LABELS_PATH)

BASE_URL = get_ngrok_url() or os.getenv("BASE_URL") or config.get("base_url")
...
raw_device = os.getenv("DEVICE_CODE", config.get("device_code", "CAM001")).strip()
...
# =========================
# REGISTER DEVICE & UPDATE
# =========================
def register_device():
    try:
        res = requests.post(REGISTER_URL, json={"cafe_id": cafe_id, "device_code": device_code}, timeout=3)
...
def update_device_status():
    ...
    try:
        requests.put(UPDATE_URL, json={"status": "active", "cafe_id": cafe_id}, timeout=1)
    except Exception as e:
        print("[ERROR UPDATE DEVICE]:", e)

# =====================================================
# PERUBAHAN 1: ROI BARU UNTUK KAMERA 2 (AGAR TIDAK BENTROK)
# =====================================================
roi_manager = ROIManager()

# Sesuaikan ID Meja (misal: Meja 5, 6, 7, 8) dan koordinat boks berdasarkan sudut pandang Kamera 2
roi_manager.add_roi(ROI(5, [(1164, 717), (64, 717), (64, 416), (1164, 416)], offset_y=50))
roi_manager.add_roi(ROI(6, [(418, 307), (0, 307), (0, 120), (418, 120)], offset_y=50))
roi_manager.add_roi(ROI(7, [(860, 202), (537, 202), (537, 3), (860, 3)], offset_y=50))
roi_manager.add_roi(ROI(8, [(1278, 307), (860, 307), (860, 53), (1278, 53)], offset_y=50))

# =========================
# STATE MACHINE
# =========================
roi_logic = {roi.roi_id: ROIStateMachine(roi.roi_id, cafe_id, device_code) for roi in roi_manager.rois}
...
# =====================================================
# PERUBAHAN 2: KAMPANYEKAN CAM_INDEX "1" KE BACKEND
# =====================================================
    ...
    try:
        ...
        requests.post(
            ...
            data={
                ...
                "cam_index": "1"  # <--- Menandakan bahwa ini adalah Kamera 2
            },
            timeout=0.5
        )
    except Exception as e:
...
def show_frame_safe(frame):
    cv2.imshow("Cafe Monitoring System - Camera 2", frame)
    cv2.waitKey(1)
    return False
...
def app_callback(pad, info, user_data):
        ...
        if class_id == 2:
            for roi_data in roi_manager.rois:
                ...
                if cv2.pointPolygonTest(poly, (float(cx), float(cy)), False) >= 0:
                    ...
                    tracked_persons_per_roi[roi_data.roi_id].append({"id": track_id, "pos": (cx, cy)})
        elif class_id == 1: 
            for roi_data in roi_manager.rois:
                poly = np.array(roi_data.points, np.int32)
                if cv2.pointPolygonTest(poly, (float(cx), float(cy)), False) >= 0:
    ...
    roi_timers, roi_stay_timers = {}, {}
    for roi_id, data in roi_results.items():
        food = data["food"]
        tracked_persons = tracked_persons_per_roi.get(roi_id, [])
        state, waiting_time, stay_time = roi_logic[roi_id].update(tracked_persons, food)
        roi_states[roi_id] = state
        roi_timers[roi_id] = waiting_time
        roi_stay_timers[roi_id] = stay_time

    roi_manager.draw_all_with_status(annotated_frame, roi_results, roi_states, roi_timers, roi_stay_timers)
    ...
    cv2.putText(annotated_frame, f"FPS:{int(fps)}", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,0,0), 2)
    frame_store.latest_frame = annotated_frame.copy()

    threading.Thread(target=upload_frame, args=(annotated_frame.copy(),), daemon=True).start()
...
if __name__ == "__main__":
    ...
    if not os.path.exists(absolute_hef_path) or not os.path.exists(absolute_labels_path):
        print("[ERROR FATAL] Model/Labels tidak ditemukan!")
        sys.exit(1)
    ...
    # =====================================================
    # PERUBAHAN 3: KUNCI INPUT KE /dev/video1 UNTUK KAMERA 2
    # =====================================================
    sys.argv = [
        "main_hailo_cam2.py",
        "--input", "/dev/video2",  # <--- Mengarah ke hardware Kamera Kedua
        "--hef-path", absolute_hef_path,
        "--labels-json", absolute_labels_path,
    ]
